...
class SummaryViewModel(
    ...
    lateinit var orderId: String //= "GW7S52T4PS210401g4uovrjvP01"
    ...
    private fun getPayMethods(
        ...
        val type = when (paymentMethod.paymentType) {
            PaymentType.BLIK_GENERIC, PaymentType.PBL, PaymentType.GOOGLE_PAY -> "PBL"
            PaymentType.CARD, PaymentType.PEX -> "CARD_TOKEN"
            PaymentType.BLIK_TOKENS, PaymentType.BLIK_AMBIGUITY -> "BLIK_TOKEN"
        }
        val value =
    ...
    fun createOrder(paymentMethod: PaymentMethod, authorizationCode: String? = null) {
        getOrderCreateRequest(paymentMethod, authorizationCode).toObservable()
            .subscribeOn(AndroidSchedulers.mainThread()).observeOn(Schedulers.io()).concatMap {
            ...
            .subscribe({ response -> handleOrderCreateResponse(response, paymentMethod) },
                { throwable ->
                    val sw = StringWriter()
    ...
    fun pay() {
        ...
        when (paymentMethod?.paymentType) {
            ...
            PaymentType.BLIK_AMBIGUITY -> {
                blikAmbiguityEvent.postValue(Unit)
