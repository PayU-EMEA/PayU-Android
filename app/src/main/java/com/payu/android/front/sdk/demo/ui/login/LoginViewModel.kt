...
class LoginViewModel(
    ...
    val email = ObservableString(persistentRepository.email)
    ...
    val termsEvent = SingleLiveEvent<Boolean>()
    ...
    private fun saveCredentialsIfNedded(posId: String, clientSecret: String, email: String) {
        ...
        if (isSaveCredentialsChecked.get()) {
            ...
            persistentRepository.email = email
        }
    }
    ...
    fun login() {
        ...
        val email = email.get()?: ""
        saveCredentialsIfNedded(posId, clientSecret, email)
        showLoginProgress.set(true)
        api.authenticate(posId, clientSecret, email, "100")
